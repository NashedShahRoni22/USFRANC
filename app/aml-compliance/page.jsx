...
const amlPolicySections = [
  ...
  },
  {
    ...
    subtitle: "Lutte contre le blanchiment d'argent (LBA) et Contrôle de Conformité",
    paragraphs: [
  ...
  },
  {
    ...
    listGroups: [
      ...
      },
      {
        ...
        items: ['Pass: Proceed with transaction', 'Flagged: Escalate for manual review'],
      },
      {
  ...
  },
  {
    ...
    listGroups: [
      ...
      {
        title: '3. Décision :',
        items: ['Aucun signalement : Autoriser la transaction', 'Signalé : Escalade au contrôle manuel'],
      },
      {
        title: '4. Archivage :',
        items: ['Conservation sécurisée des documents, rapports et validations.'],
      },
      {
        title: '5. Déclaration d’activité suspecte :',
        items: ['Déposer un rapport (SAR) auprès de l’unité de renseignement financier (ex. : TRACFIN, FinCEN).'],
      },
    ],
  },
];
...
export default function AMLPolicyPage() {
  return (
    <Container>
      ...
      <div className="space-y-10 mb-8 md:mb-16">
        {amlPolicySections.map((section) => (
          <div key={section.id} className="bg-white p-6 rounded-lg shadow border border-gray-200">
            <h2 className="text-2xl font-semibold text-logo mb-1">{section.title}</h2>
            ...
            {section.footer && <p className="text-gray-700">{section.footer}</p>}
